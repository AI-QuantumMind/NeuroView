...
function MRIDashboard() {
  ...
  const sideBarClasses = isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200";
  const viewerBgClasses = isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-300";
  ...
  useEffect(() => {
    const fetchPatients = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:8000/api/doctor/${doctorId}/patients`);
  ...
  const handleRender = async () => {
    if (selectedInputType === "nii.gz") {
      ...
      try {
        if (mriNv) {
          const filename = selectedImageType === "t1ce" ? t1ceFileName : flairFileName;
  ...
  const handleGenerateReport = async () => {
    ...
    try {
      ...
      // Load JSON file
      const jsonResponse = await fetch("./backend/VisionModel/report/mri_report.json");
      if (!jsonResponse.ok) {
      ...
      setPatients((prev) =>
        prev.map((p) =>
          p._id === selectedPatient
            ? {
                ...p,
                report_ids: Array.isArray(p.report_ids) ? [...p.report_ids, data.report_id] : [data.report_id],
              }
            : p
        )
      );
            ...
            <>
              <div className="mb-6">
                ...
                >
                  ...
                  <p className={`text-sm ${textPrimary}`}>Browse / Drop T1CE file</p>
                </div>
                {t1ceFileName && (
                  <div className="mt-2 text-sm text-green-500">T1CE uploaded: {t1ceFileName}</div>
                )}
              </div>
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  <p className={`text-sm ${textPrimary}`}>Browse / Drop FLAIR file</p>
                </div>
                {flairFileName && (
                  <div className="mt-2 text-sm text-green-500">FLAIR uploaded: {flairFileName}</div>
                )}
              </div>
            </>
          )}
          ...
          {selectedInputType === "image" && (
            <div className="mb-6">
              ...
              >
                ...
                <p className={`text-sm ${textPrimary}`}>Browse / Drop image file</p>
              </div>
            </div>
          )}
          ...
          <div className="mb-6">
            ...
            <button
              ...
              className={`mt-4 w-full py-2 text-white rounded ${
                isProcessing ? "bg-blue-300 cursor-not-allowed" : "bg-blue-500 hover:bg-blue-600"
          ...
          {role === "doctor" && (
            <div className="mb-6">
              ...
              <button
                ...
                className={`mt-4 w-full py-2 text-white rounded ${
                  isGeneratingReport ? "bg-gray-400 cursor-not-allowed" : "bg-purple-500 hover:bg-purple-600"
                } transition-all duration-300`}
              >
                {isGeneratingReport ? "Generating Report..." : "Generate & Assign Report"}
              </button>
            </div>
          )}
